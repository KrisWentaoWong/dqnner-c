...
function nql:setLRFromServer(lrFromServer)
    self.lrFromServer = lrFromServer
end
...
function nql:getQUpdate(args)
    ...
    -- Double DQN 的target_q计算
--    local q_max_index = self.network:forward(s2):argmax(2)
--    print("q_max_index", q_max_index)
--    q2_max = target_q_net:forward(s2):gather(1,q_max_index).detach()
--    print("q2_max", q2_max)
...
function nql:qLearnMinibatch()
    ...
    if self.lrFromServer>0 then self.lr = self.lrFromServer end
...
function nql:perceive(reward, rawstate, terminal, stepSeqForCurrentMention, isNullByLastQuery, isHumanByLastQuery, hasQueriedHuman, testing, testing_ep)      ---lutm: This function is called twice in train_agent.lua.
    ...
    local actionIndex = 1
    local objectIndex = 1
--    print("stepSeqForCurrentMention", stepSeqForCurrentMention)
--    print("isNullByLastQuery", isNullByLastQuery)
--    print("isHumanByLastQuery", isHumanByLastQuery)
--    print("hasQueriedHuman", hasQueriedHuman)
    if not terminal then
        actionIndex, objectIndex = self:eGreedy(curState, testing_ep)
        -- forced action
         if stepSeqForCurrentMention>= 10 then
--             -> finish
             actionIndex = 2
             objectIndex = self.n_objects
         end
        -- if isHumanByLastQuery then
            -- -> accept
            -- actionIndex = 1
            -- objectIndex = 5
--            print("**************************** -> accept")
        -- elseif hasQueriedHuman then
            -- -> finish
            -- actionIndex = 3
            -- objectIndex = 5
--            print("**************************** -> finish")
        -- elseif stepSeqForCurrentMention>= 10 then
            -- -> finish
            -- actionIndex = 3
            -- objectIndex = 5
--            print("**************************** -> finish")
        -- elseif isNullByLastQuery then
            -- accept -> reject
